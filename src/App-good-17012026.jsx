...
import { Menu, X, ArrowRight, CheckCircle, HardHat, Home, PenTool, Truck, Users, MessageCircle, Send, Facebook, Youtube, Linkedin, Instagram,Lock } from 'lucide-react';
...
const App = () => {
  ...
  // NOUVEAU : État pour cacher/montrer l'admin
  const [showAdmin, setShowAdmin] = useState(false);
  
  // Compteur pour le passage secret (Triple clic)
  const [secretClicks, setSecretClicks] = useState(0);

  // Fonction pour gérer le code secret (3 clics rapides sur le footer)
  const handleSecretClick = () => {
    setSecretClicks(prev => prev + 1);
    // Au bout de 3 clics, on ouvre la porte
    if (secretClicks + 1 === 3) {
      setShowAdmin(true);
      setSecretClicks(0); // On remet à zéro
    }
    // Si on arrête de cliquer pendant 2 secondes, on remet à zéro
...
{/* NAVIGATION ORDI */}
      ...
      */}
      ...
      {/* --- PORTAIL ADMIN (INVISIBLE PAR DÉFAUT) --- */}
      {showAdmin && (
        <div className="fixed inset-0 z-[100] bg-white overflow-y-auto animate-fadeIn">
          {/* Bouton pour fermer et revenir au site */}
          <button 
            onClick={() => setShowAdmin(false)} 
            className="fixed top-4 right-4 bg-red-600 text-white px-4 py-2 rounded-full font-bold shadow-lg z-50 hover:bg-red-700 transition"
          >
            Fermer X
          </button>
      ...
      <section className="bg-gray-900 text-white pt-16 pb-8">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-12 mb-12">
           ...
           {/* COLONNE 3 : RÉSEAUX SOCIAUX (Corrigée avec Texte + Icônes) */}
          <div>
            ...
            {/* Zone des icônes dynamiques (Rondes et Fixes) */}
            <div className="flex gap-4 items-center">
              ...
              {socials.facebook && (
                <a 
                  ...
                  className="w-10 h-10 flex items-center justify-center bg-gray-800 rounded-full hover:bg-blue-600 transition duration-300"
                >
                  <Facebook size={20} />
                </a>
              )}
              ...
              {socials.linkedin && (
                <a 
                  ...
                  target="_blank" 
                  rel="noopener noreferrer" 
                  className="w-10 h-10 flex items-center justify-center bg-gray-800 rounded-full hover:bg-blue-700 transition duration-300"
                >
                  <Linkedin size={20} />
                </a>
              )}
              ...
              {socials.instagram && (
                <a 
                  ...
                  target="_blank" 
                  rel="noopener noreferrer" 
                  className="w-10 h-10 flex items-center justify-center bg-gray-800 rounded-full hover:bg-pink-600 transition duration-300"
                >
                  <Instagram size={20} />
                </a>
              )}
        ...
        <div className="border-t border-gray-800 mt-12 pt-8 text-center text-gray-500 text-sm">
          <p 
             ...
             className="cursor-default select-none hover:text-gray-400 transition"
             title="Tous droits réservés" // Astuce pour ne pas éveiller les soupçons
          >
